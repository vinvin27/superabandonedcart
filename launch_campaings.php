...
class LaunchCampaign
{
    public function sendCampaign()
    {
        ...
        $sql = "SELECT * FROM (
		SELECT
		CONCAT(LEFT(c.`firstname`, 1), '. ', c.`lastname`) `customer`, a.id_cart total, ca.name carrier, c.id_customer, a.id_cart, a.date_upd,a.date_add, c.newsletter,
        ...
        foreach ($abandoned_carts as $abncart) {
            ...
            foreach ($allCampaigns as $camp) {
                ...
                if ($camp['newsletter_only'] == 1 && $abncart['newsletter'] == 0) {
                    $cartIsOnCampaign = false;
                    //ddd('failed');
                    if (DEBUG_SAC) {
                        echo 'Newsleter check failed : '.$abncart['id_customer'].' - IdCart : '.$abncart['id_cart'].'<br/>';
                    }
                }
                if ($cartIsOnCampaign) {
                    if (DEBUG_SAC) {
