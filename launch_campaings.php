...
foreach( $abandoned_carts as $abncart ) {
	...
	foreach( $allCampaigns as $camp ){
		...
		$campaign = new Campaign($camp['id_campaign']);
		$cartIsOnCampaign = checkIfCartIsOnCampaign( $abncart['date_add'] , $camp['execution_time_day'] , $camp['execution_time_hour']);
		if( $cartIsOnCampaign ){
			...
			foreach( $products as $prod ){
				...
				$cart_content .= '<tr >
									...
									<td align="center" ><a href="'.$link->getProductLink($p).'"/>'.$p->name.'</a>e</td>
			...
			$tpl_vars = array(
				...
				'{coupon_valid}' => date('d/m/Y',strtotime( $cR->date_to )),
