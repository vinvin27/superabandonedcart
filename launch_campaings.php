			...
			foreach( $allCampaigns as $camp ) {
				...
				if( $cartIsOnCampaign ){
					...
					$tpl_vars = array(
						...
						'{order_link}' => Context::getContext()->link->getPageLink( 'order', false,  (int)Context::getContext()->cart->id_lang,'step=3&recover_cart='.(int)$abncart['id_cart'].'&token_cart='.md5(_COOKIE_KEY_.'recover_cart_'.(int)$abncart['id_cart']).'?id_cart='.(int)$abncart['id_cart'].'&id_customer='.(int)$abncart['id_customer']),
